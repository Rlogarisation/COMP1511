...
void pad_left(struct character_node *characters, char pad_character) {
    // Create a temporary pointer current.
    struct character_node *current = characters;
    struct character_node *previous = characters;
    char previousData = pad_character;
    char currentData = pad_character;
    // Locate the position of last character in linked list,
    // then loop forward, move each character one position back.
    while (current != NULL) {
        ...
        if (previous == current) {
            ...
            // Assign data into temporary char.
            previousData = previous->data;
            currentData = current->data;
            // Move the data backward, add pad_character into first.
            previous->data = pad_character;
