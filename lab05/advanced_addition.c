...
int sum(int num_lines, int num_digits, int array[MAX_SIZE][MAX_SIZE]) {
    int carryover = 0;
    int coutnerForNumDigits = num_digits - 1;
    // Consider from the right hand side first
    while (coutnerForNumDigits >= 0) {
        int counterForNumLines = 0;
        // Considering each row.
        while (counterForNumLines < num_lines) {
            array[num_lines][coutnerForNumDigits] += 
            array[counterForNumLines][coutnerForNumDigits];

            if (array[num_lines][coutnerForNumDigits] < 10) {
                counterForNumLines++;
            } else if (array[num_lines][coutnerForNumDigits] >= 10) {
                // -10 first
                array[num_lines][coutnerForNumDigits] -= 10;
                // If left boundary is reached.
                if (coutnerForNumDigits - 1 < 0) {
                    // move 10 to left
                    carryover += 1;
                } else {
                    array[num_lines][coutnerForNumDigits - 1] += 1;
                }
                counterForNumLines++;
            }
