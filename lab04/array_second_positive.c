...
int array_second_positive(int size, int array[size]) {
    ...
    int counterForPositiveNumber = 0;
    int arrayForPositiveNumber[size];
    while (counterForArray < size) {
        if (array[counterForArray] > 0) {
            arrayForPositiveNumber[counterForPositiveNumber] = 
            array[counterForArray];

            counterForPositiveNumber++;
        }
        counterForArray++;
    }
    if (counterForPositiveNumber >= 2) {
        return arrayForPositiveNumber[1];
    } else {
